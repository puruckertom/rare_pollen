...
list.of.packages <- c("ggplot2", "dplyr","nlme","lme4")
...
for(i in 1:length(interesting_neonics)){
  fm <- lme(as.formula(paste("mortality ~ ",interesting_neonics[i],
            sep="")),random=~1|site_code, correlation=corAR1(form=~time.block|site_code),
  ...
  #Examine temporal autocorrelation function 
  layout(matrix(1:2))
  acf(resid(fm))
