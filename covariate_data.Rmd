...
Let's see if we find any correlations between %corn and mortality

```{r eval=TRUE, echo=TRUE}
attach(osu_covariate)
cor.test(~mortality+per_corn)
...
for(i in 1:length(interesting_neonics_binomial)){
  plot(factor(osu_covariate[,interesting_neonics_binomial[i]],levels = c(0,1),labels=c("No","Yes")),per_corn,pch=19,main=paste("Detection of",gsub("_detected","",interesting_neonics_binomial[i]),"vs % corn"),xlab=paste(gsub("_detected","",interesting_neonics_binomial[i]),"detected?"),ylab="Percent corn")
}
...
for(i in 1:length(interesting_neonics)){
  ...
  print(summary(fm))
  #hist(resid(fm))
  g <- ggplot(osu_covariate,aes_string(interesting_neonics[i],"mortality"))+geom_point(na.rm=T)+
    geom_smooth(method="lm",formula=y~x,na.rm=T)+theme_bw()+
...
#####Let's graph corn versus these neonics, by planting period

```{r eval=TRUE, echo=TRUE}

for(i in 1:length(interesting_neonics)){
  g <- ggplot(osu_covariate,aes_string("per_corn",interesting_neonics[i]))+geom_point(na.rm=T)+
...
for(i in 1:length(interesting_neonics_binomial)){
  fm <- lm(as.formula(paste("mortality ~ ",interesting_neonics_binomial[i],
                            sep="")),data=osu_covariate)
...
for(i in 1:length(interesting_neonics_binomial)){
  fm <- glm(as.formula(paste(interesting_neonics_binomial[i],"~ per_corn * planting",
...
##### We don't see any significant relationships, however we saw above that the corn effect was only during planting periods, and these two neonics have 100% detection rate during planting periods. Therefore, we can't get a good estimate of the corn effect during this critical time. 
<br><br>
...
for(i in 1:length(neonics_to_eval)){
  fm <- glm(as.formula(paste(neonics_to_eval[i],"~ per_corn * planting", #Note: not using side ID b/c there are many singularities.
                            sep="")),data=osu_covariate,family=binomial(link="logit"))
...
#####No significant relationships found so far for the other neonics in pollen, or anything in dead bees...
<br><br>
...
```{r eval=TRUE,echo=TRUE}
fm <- lm(mortality~per_corn*planting,data=osu_covariate)
summary(fm)
