...
If the interaction is significant, it will indicate that effect of corn nearby on neonic conc. depends on whether it is during a planting period. 
...
Summary: There was no effect of percent corn nearby on clothianidin, thiamethoxam, or clo + thi. 
<br><br>
...
Summary: Percent corn nearby was NOT significantly related to average May or planting period neonic concentrations. (Note: The positive effect of percent corn nearby on planting period mean clo + thi was borderline significant - p=0.053)
...
Summary: For clothianidin and clo + thi, mean concentrations were greater during the planting period than the monthly average. For thiamethoxam, the difference was borderline significant (p=0.06)
...
* There was no significant effect of percent corn nearby on daily or mean neonic concentrations in pollen. 
...
If the pollen change covariate was not significant, we dropped the term and refit the model. Note: pollen covariate not applicable when rel. change in pollen area was the main variable of interest. 
...
```{r echo=T,eval=T,fig.width=5,fig.height=4}
...
g <- ggplot(site_wide,aes(mean_ct_may,open_relchg_3))+geom_point(na.rm=T,size=2.5)+
    geom_smooth(method="lm",formula=y~x,na.rm=T,size=2)+theme_bw()+ 
    theme(panel.grid.major=element_blank(),panel.grid.minor=element_blank()) +
    labs(x="Mean clo + thi in pollen in May (ng g-1)",
         y="Rel. change in open brood area (%)")+geom_hline(yintercept=0) +
  theme(plot.title = element_text(hjust = 0.5))
...
* However, there may have been a recovery in the second time interval (late May to late June), as hives exposed to greater neonic levels in May had a LARGER increase in bee population during this time (for seams of bees only).

* Finally, in the third time interval (June to August), hives located in areas where neonic concentrations were highest in May had a greater increase in pollen area. This could be potentially be related to corn crop flowering?
